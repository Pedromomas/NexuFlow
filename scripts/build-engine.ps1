...
& $Py -c "import pathlib,nexus_engine; assert nexus_engine.__version__ == '1.5.5', nexus_engine.__version__; assert pathlib.Path(nexus_engine.__file__).resolve().is_relative_to(pathlib.Path(r'$Root\engine\src').resolve()), nexus_engine.__file__"
